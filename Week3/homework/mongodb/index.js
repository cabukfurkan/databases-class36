...
async function findEpisodesExercises(client) {
  ...
  const cursor3 = await client.db('databaseWeek3')
    .collection("bob_ross_episodes").find({ elements: "CLIFF" })
  const result3 = await cursor3.toArray()
  const titles3 = await result3.map(x => x.title)
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${titles3}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
  const cursor4 = await client.db('databaseWeek3')
    .collection("bob_ross_episodes").find({ $and: [{ elements: "CLIFF" }, { elements: "LIGHTHOUSE" }] })
  const result4 = await cursor4.toArray()
  const titles4 = await result4.map(x => x.title)
  console.log(
...
async function updateEpisodeExercises(client) {
  ...
  console.log(
    `Ran a command to update episode 13 in season 30 and it updated ${result1.matchedCount} episodes`
  );
